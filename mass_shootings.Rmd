...
#2020
ms_pages_2020 <- 
    tibble(pages = c("https://www.gunviolencearchive.org/reports/mass-shooting?year=2020",
                                  str_c("https://www.gunviolencearchive.org/reports/mass-shooting?page=",c(1:24),"&year=2020")))

ms_scrape_2020 <- ms_pages_2020 %>%
    mutate(scrape = map(.x = pages, .f = ~read_html(.x)))

ms_2020_tbl <- map_df(ms_scrape_2020$scrape, ~ html_table(.x))

#2021
ms_pages_2021 <- 
    tibble(pages = c("https://www.gunviolencearchive.org/reports/mass-shooting?year=2021",
                                  str_c("https://www.gunviolencearchive.org/reports/mass-shooting?page=",c(1:27),"&year=2021")))

ms_scrape_2021 <- ms_pages_2021 %>%
    mutate(scrape = map(.x = pages, .f = ~read_html(.x)))

ms_2021_tbl <- map_df(ms_scrape_2021$scrape, ~ html_table(.x))

#2022
ms_pages_2022 <- 
    tibble(pages = c("https://www.gunviolencearchive.org/reports/mass-shooting?year=2022",
                                  str_c("https://www.gunviolencearchive.org/reports/mass-shooting?page=",c(1:25),"&year=2022")))

ms_scrape_2022 <- ms_pages_2022 %>%
    mutate(scrape = map(.x = pages, .f = ~read_html(.x)))

ms_2022_tbl <- map_df(ms_scrape_2022$scrape, ~ html_table(.x))

#2023
ms_pages_2023 <- 
    tibble(pages = c("https://www.gunviolencearchive.org/reports/mass-shooting?year=2023",
                                  str_c("https://www.gunviolencearchive.org/reports/mass-shooting?page=",c(1:26),"&year=2023")))

ms_scrape_2023 <- ms_pages_2023 %>%
    mutate(scrape = map(.x = pages, .f = ~read_html(.x)))

ms_2023_tbl <- map_df(ms_scrape_2023$scrape, ~ html_table(.x))
...
# 
# 
# ms_scrape_others <- ms_pages_others %>%
#     mutate(scrape = map(.x = pages, .f = ~read_html(.x)))
# 
# ms_others_tbl <- map_df(ms_scrape_others$scrape, ~ html_table(.x))
```
...
complete_tbl <- ms_2014_tbl |> 
    ...
    bind_rows(ms_2020_tbl) |> 
    bind_rows(ms_2021_tbl) |> 
    bind_rows(ms_2022_tbl) |> 
    bind_rows(ms_2023_tbl) |> 
    #bind_rows(ms_others_tbl) %>%
    distinct()
...
group_function <- function(df, ...) {
    ...
    output <- df %>% 
        ...
        summarize(n_incidents = n(),
                  n_deaths = sum(victims_killed + suspects_killed, na.rm = T),
                  n_injuries = sum(victims_injured + suspects_injured, na.rm = T),
                  .groups = "drop")
...
Unfortunately, every US state has had a mass shooting.  When this project was completed at the end of 2022, there were two states that had not had a mass shooting since 2014.
...
per_capita_tbl %>% 
    ...
    theme_tq() +
    scale_fill_tq() +
    labs(title = "Per Capita Mass Shootings, Deaths, and Injuries by US State in 2023",
         x = "",
...
top_cities <- ms_tbl %>% 
    ...
    summarize(number_incidents = n(),
               number_killed = sum(victims_killed + suspects_killed),
              number_injured = sum(victims_injured + suspects_injured),
              .groups = "drop") %>% 
...
top_cities_incidents %>% 
    ggplot(aes(factor(year), number_incidents, fill = city_or_county)) +
    geom_col(alpha = 0.9) +
...
driver = webdriver.Chrome(options=options) # this establishes the browser
